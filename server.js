...
function MongoSink() {
  MongoClient.connect(url, { useNewUrlParser: true }, async (err, db) => {
    ...
    this._model = dbo.collection("info");
  });
}
const mongo = new MongoSink()

const reportError = (req, res, err) => {
  logger.error('reportError req.originalUrl', req.originalUrl, "err", err)
  return res.status(500).end({ err: err.stack || err.toString() })
}

const reportInvokeError = (req, res, err) => {
  logger.error('reportInvokeError req.originalUrl', req.originalUrl, "err", err)
...
const writeResponse = (res, data) => {
  if (Object.prototype.toString.call(data) !== '[object Object]') {
    ...
    return res.status(500).end(JSON.stringify(obj))
  } else {
          ...
          try {
            ...
            return mongo.find({ username }).toArray(function (err, result) {
              ...
              if (!result.length) {
                ...
                return reportInvokeError(req, res, "用户名错误")
              } else {
            ...
            jwt.verify(token, secret, (err, decoded) => {
              if (err) {
                ...
                return mongo.find({ username }).toArray(function (err, result) {
                  ...
                  const lastDay = result[0].date[result[0].date.length - 1];
                  Object.assign(sendData, { lastDay });
                  logger.debug("last_sign  sendData", sendData);
                  const obj = { result: sendData }
                  return writeResponse(res, obj)
                });
              }
